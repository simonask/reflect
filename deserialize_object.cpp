...
namespace {
	...
	const StructTypeBase* get_class_from_map(const ArchiveNode& node, std::string& out_error) {
		std::string clsname;
		if (!node["class"].get(clsname)) {
			out_error = "Class not specified.";
			return nullptr;
		}
		const StructTypeBase* struct_type = TypeRegistry::get(clsname);
	...
	const DerivedType* transform_if_composite_type(const ArchiveNode& node, const StructTypeBase* base_type, std::string& out_error) {
	...
	const DerivedType* get_type_from_map(const ArchiveNode& node, std::string& out_error) {
		const StructTypeBase* struct_type = get_class_from_map(node, out_error);
		if (struct_type != nullptr) {
